...
function Banking() {
  ...
  const loadSchemes = () => {
    ...
    axios.get("http://localhost:9000/schemes")
      .then((res) => {
        // Filter only banking schemes
        const bankingSchemes = res.data.filter(scheme => scheme.category === 'banking');
        setSchemes(bankingSchemes);
        setLoading(false);
      })
      .catch((err) => {
  ...
  return (
    <div className="page-content">
      <div className="page-header">
        ...
        <h1>Banking Schemes</h1>
        <p>Explore government schemes and programs for banking sector.</p>
      </div>
      ...
      <div className="search-container">
        <div className="search-box">
          ...
          <input
            type="text"
            placeholder="Search banking schemes..."
            value={searchTerm}
      ...
      {!loading && !error && schemes.length === 0 && (
        <div className="no-schemes">
          <h3>No Banking Schemes Available</h3>
          <p>There are currently no banking schemes in our database.</p>
        </div>
      )}
      ...
      {!loading && !error && schemes.length > 0 && filteredSchemes.length === 0 && searchTerm && (
        <div className="no-schemes">
          ...
          <p>No banking schemes match your search for "{searchTerm}".</p>
